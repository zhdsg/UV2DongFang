     ...
     try {
       ...
       val xiaoChengXu =dataRDD.filter(f=>f.getString(2).equals("sharexcx")||f.getString(2).equals("xiaochengxu_dftt"))
        ...
        var dataTenMinRDD =unionRDD.filter(
         ...
         unionRDD.unpersist()
      // val pv_sdk_10min =dataTenMinRDD.filter(f=>f.getString(2).equals("sdk_total")).count()
        //公共RDD持久化
...
//        dataRDD=dataRDD.unpersist()
       ...
       //activeRDD4Active.persist()
       val sdkRDD4Active = activeRDD4Active.filter(f=>{
         var flag =false
         ...
         flag
       }).map(f => Row(f.getLong(0),f.getString(1),"sdk_total",f.getString(3))).coalesce(50)
       val xiaoChengXuActive =activeRDD4Active.filter(f=>f.getString(2).equals("sharexcx")||f.getString(2).equals("xiaochengxu_dftt"))
         .map(f => Row(f.getLong(0),f.getString(1),"小程序汇总",f.getString(3))).coalesce(60)
       val unionRDD4Active =activeRDD4Active.union(sdkRDD4Active).union(xiaoChengXuActive)
       //activeRDD4Active.unpersist()
...
//           s" |activecnt:${total_UV_active} |incr_activecnt:${total_UV_10m_active}")
  ...
  def getData(spark:SparkSession  ,dt:String,dateLineDown:Long,dateLineUp:Long):RDD[(Row)]={
    ...
    //  s"case when qid='huitt' then clientime else uid end as uid  " +
      ...
      s"${ConfigurationManager.getString(Constract.HIVE_H5_TABLE)} " +
      s"where dt ='${dt}' " +
        s"AND((logtype<>'log_toutiao_sdk_active') OR (logtype='log_toutiao_sdk_active' and urlto regexp  'http'))"
        +
  ...
  .filter(row => ETLUtil.filter(row.getString(2)))
      .map(
        cur => {
              ...
              var clientime =cur.getString(5)
              if(clientime ==null ){
                clientime="null"
              }
              if(qid.toLowerCase().contains("huitt")&& !clientime.equals("null")){
                uid =clientime
              }
